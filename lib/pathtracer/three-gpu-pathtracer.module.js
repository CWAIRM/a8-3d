// three-gpu-pathtracer 0.0.24 (MIT, https://github.com/gkjohnson/three-gpu-pathtracer), vendored for the A8 site.
// (vendored patch) the two imports below were rewritten from 'three-mesh-bvh' and 'three/examples/jsm/postprocessing/Pass.js';
// every other change is marked '(vendored patch)' — full list in assets/photo/CREDITS.md
...
function mergeGeometries( geometries, options = {}, targetGeometry = new BufferGeometry() ) {
	...
	for ( let i = 0, l = attributes.length; i < l; i ++ ) {
		...
		if ( ! targetGeometry.getAttribute( key ) ) {
			...
			// (vendored patch, A8 photo mode) the merged color attribute is always float RGBA, whatever the first mesh had
			targetGeometry.setAttribute( key, key === 'color' ?
				new BufferAttribute( new Float32Array( 4 * totalAttributeCount ), 4, false ) :
				createAttributeClone( firstAttr, totalAttributeCount ) );
			...
			const attr = geometry.getAttribute( key );
 			if ( ! skip ) {
				...
				if ( key === 'color' && (
					...
					// (vendored patch, A8 photo mode) the original loop copied the wrong way round (target → source)
					// over the wrong index range, so RGB vertex colours merged next to RGBA ones came out black
					for ( let i = 0, l = attr.count; i < l; i ++ ) {

						targetAttribute.setXYZW( offset + i, attr.getX( i ), attr.getY( i ), attr.getZ( i ), attr.itemSize >= 4 ? attr.getW( i ) : 1.0 );
	...
	generate( onProgress = null ) {
		...
		if ( this.generateBVH ) {
			...
			if ( result.changeType === GEOMETRY_REBUILT ) {
				...
				const bvhOptions = {
					strategy: SAH,
					targetLeafSize: 1,   // (vendored patch) three-mesh-bvh 0.9 renamed maxLeafTris → targetLeafSize
					indirect: true,
					onProgress,
					...this.bvhOptions,
				};
...
class RenderTarget2DArray extends WebGLArrayRenderTarget {
	...
	constructor( width, height, options ) {
		...
		// (vendored patch, A8 photo mode) back-reference so the owner can dispose the whole render target:
		// disposing only this texture leaves three's framebuffer-owned GL texture alive (texture array ~50–120 MB)
		this.texture.renderTarget2DArray = this;
...
const attenuate_hit_function = /* glsl */`
	...
	// (vendored patch, A8 photo mode) thin, clear, untextured glass
	bool photoIsClearGlass( Material material ) {

		return material.thinFilm && material.transmission > 0.5 && material.transmissionMap == - 1 &&
			material.roughness < 0.05 && material.roughnessMap == - 1 &&
			material.metalness < 0.05 && material.map == - 1 && ! material.transparent;
	...
	bool attenuateHit(
		...
		for ( int i = 0; i < traversals; i ++ ) {
			...
			if ( hitType == FOG_HIT ) {
				...
				// (vendored patch) with window portals on, environment shadow rays stop at clear glass
				if ( photoEnvShadow && photoPortals > 0.5 && photoIsClearGlass( material ) ) {
...
const direct_light_contribution_function = /*glsl*/`
	...
	vec3 directLightContribution( vec3 worldWo, SurfaceRecord surf, RenderState state, vec3 rayOrigin ) {
		...
		if( lightsDenom != 0.0 && rand( 5 ) < float( lights.count ) / lightsDenom ) {
			...
			vec3 attenuatedColor;
			photoEnvShadow = true; // (vendored patch) see photoPortals
			bool envOccluded = envPdf <= 0.0 || ! isDirectionValid( envDirection, surf.normal, surf.faceNormal ) ||
				attenuateHit( state, envRay, INFINITY, attenuatedColor );
			photoEnvShadow = false;
			if ( ! envOccluded ) {
...
const render_structs = /* glsl */`
	...
	struct RenderState {
		...
		bool throughGlass; // (vendored patch) an indirect ray passed through thin clear glass
	...
	constructor( parameters ) {
		...
		super( {
			...
			uniforms: {
				...
				photoPortals: { value: 0.0 }, // (vendored patch, A8 photo mode)
			...
			fragmentShader: /* glsl */`
				...
				// (vendored patch, A8 photo mode) window portals: when photoPortals is on, the sky seen through thin
				// clear glass is supplied by rect-area "portal" lights placed on the inside of each window, so the
				// environment map must not also be counted through that glass (neither by next-event estimation
				// shadow rays nor by bounce rays that escape through a pane).
				uniform float photoPortals;
				bool photoEnvShadow = false;
				...
				void main() {
					...
					for ( int i = 0; i < bounces; i ++ ) {
						...
						if ( hitType == NO_HIT ) {
							...
							} else if ( photoPortals > 0.5 && state.throughGlass ) {

								// (vendored patch) this sky was already counted through the window portal lights
						...
						// (vendored patch, A8 photo mode) thin, clear, untextured glass: indirect rays pass straight
						// through, exactly like shadow rays already do in attenuateHit(). Without this the MIS weight
						// of whatever the ray reaches behind the pane (sky, sun disc, area light) is computed from the
						// glass' transmission "pdf" instead of the previous real bounce, so light behind windows is
						// counted twice (next-event estimation + BSDF hit) and small bright lights turn into fireflies.
						// Camera rays (firstRay / transmissiveRay) keep the full glass BSDF, so reflections stay.
						if ( ! state.firstRay && ! state.transmissiveRay && photoIsClearGlass( material ) ) {

							state.throughGlass = true;

							if ( surfaceHit.side == 1.0 ) {

								state.throughputColor *= mix( vec3( 1.0 ), material.color, material.transmission );
						...
						// (vendored patch, A8 photo mode) also when the sampled direction is NaN: a thin-film refraction
						// at grazing angles can hit total internal reflection, refract() returns 0 and normalize(0) = NaN
						if ( scatterRec.pdf <= 0.0 || any( isnan( scatterRec.direction ) ) || ! isDirectionValid( scatterRec.direction, surf.normal, surf.faceNormal ) ) {
					...
					// (vendored patch, A8 photo mode) never let a NaN / Inf sample into the running average:
					// once a pixel of the accumulation target is NaN it stays NaN forever (shows as a black dot).
					if ( any( isnan( gl_FragColor ) ) || any( isinf( gl_FragColor ) ) ) {
...
class PathTracingRenderer {
	...
	compileMaterial() {
		...
		// (vendored patch) 編譯時先綁定路徑追蹤用的 render target：three.js 的 program 參數
		// （outputColorSpace／toneMapping）會隨「畫到畫布」或「畫到 target」而不同，
		// 不綁定的話這支巨大的 shader 會被編譯兩次（一次 compileAsync、一次真正渲染時再同步編譯）。
		const renderer = this._renderer;
		const prevTarget = renderer.getRenderTarget();
		renderer.setRenderTarget( this._primaryTarget );
		const promise = renderer.compileAsync( this._fsQuad._mesh );
