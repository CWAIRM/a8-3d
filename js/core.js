...
/* ---------- 尺寸（2026-09-19 v5 起依建商「有標公分尺寸」的平面圖 ref/plan_11_dims.jpg；
   標註沒寫到的牆沿用 layout/spec_final.json（兩位讀圖者＋裁判），逐條對照見 layout/spec_v5_dims.md）----------
   室內 27 坪 = 89.26 ㎡（外牆中心線）／ 工作陽台 1.8 坪
   樓高 H = 3.25 m（屋主提供）
   座標：原點 = 西向外牆內面 × 北向外牆內面；X 向東（平面圖右）、Z 向南（平面圖下）、Y 向上；單位公尺
   室內淨盒 8.95 × 10.35 m（圖上：北 286.5+12+248+12+336.5 ＝ 南 307.5+15+279+12+281.5 ＝ 895 cm；東 820+10+205 ＝ 1035 cm） */
export const H  = 3.25;            // 樓高
export const TW = 0.15;            // 外牆窗牆厚（北、西）；RC 牆另見 arch.js 逐道尺寸
export const IW = 0.12;            // 隔間牆厚（圖上標 12 cm）
export const W  = 8.95, D = 10.35; // 室內淨盒（東牆內面、南牆內面）
export const EYE = 1.62;           // 眼睛高度

/* ---------- 房間表（公尺）— 逐間對照 layout/spec_v5_dims.md，⛔ 不要憑印象改格局 ----------
   rects = 淨地板矩形（牆面到牆面）；area = 淨面積（扣管道間）；
...
export const R = {
  '客廳'   : room([[5.585, 0.000, 8.950, 4.040]], 13.59, [7.0, 2.2], [8.10, 3.62], [5.85, 1.00]),   // 電視在西牆、沙發靠東牆（2026-09-19 Andy：對調）
  '餐廳'   : room([[4.850, 4.040, 8.950, 8.200]], 17.06, [7.0, 6.2], [8.35, 7.55], [6.2, 5.5]),
  '主臥室' : room([[0.000, 0.000, 2.865, 4.040]], 11.57, [1.4, 2.0], [2.40, 3.60], [0.9, 1.1]),
  '臥室二' : room([[2.985, 0.000, 5.465, 3.020]],  7.49, [4.2, 1.5], [4.95, 2.60], [3.9, 0.7]),   // 上排中間那間（主題房：theme_br2.js）
  '臥室三' : room([[0.000, 5.810, 3.214, 8.200], [3.214, 6.426, 4.700, 8.200]], 10.32, [1.6, 7.1], [4.20, 7.72], [0.9, 6.9]), // 西南 L 形那間（主題房：theme_br3.js）
  '主浴廁' : room([[0.000, 4.160, 3.214, 5.690]],  4.60, [1.6, 4.95], [2.85, 5.27], [0.7, 4.95]),
  '次浴廁' : room([[6.135, 8.300, 7.736, 10.350], [7.736, 9.116, 8.950, 10.350]], 4.45, [7.0, 9.35], [6.95, 8.55], [7.75, 10.07, 0.9]),   // look 第三個數＝視線高度（稍微往下看，馬桶、洗手台、淋浴間才進得了畫面）
  '廚房'   : room([[3.225, 8.320, 6.015, 10.350]],  5.66, [4.6, 9.35], [5.52, 8.85], [3.9, 9.87]),
  '陽台'   : room([[0.000, 8.350, 3.075, 10.025]],  5.15, [1.5, 9.2], [2.55, 9.20], [0.8, 9.6]),
  '走道'   : room([[2.865, 3.140, 5.585, 4.040]],  2.45, [4.2, 3.59], [4.90, 3.59], [3.0, 3.59], false),
  '前室'   : room([[3.364, 4.160, 4.700, 6.289]],  2.84, [4.0, 5.25], [4.30, 5.25], [3.4, 5.25], false),
};
